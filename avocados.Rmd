...
We can see in these graphs that there is a cyclical trend that has emerged for the price of avocados
regardless of type. The average price of avocados dip around the new year and are most expensive
during the summer. This also corresponds to a yearly increase in avocado consumption around
the new year.
...
Reported statistics say that the majority of avocados consumed in the U.S. are produced
either in Mexico or California. Based on that information, I would expect that regions
along the southern and western US borders have the lowest distribution costs and therefore
lower prices. 
...
p1 <- ggplot(aes(x = reorder(region, AveragePrice), y = AveragePrice), data = av_city_org) +
  geom_bar(stat = 'summary', fun.y = mean) +
  xlab('City') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
p2<- ggplot(aes(x = reorder(region, AveragePrice), y = AveragePrice), data = av_city_con) +
  geom_bar(stat = 'summary', fun.y = mean) +
  xlab('City') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
